...
RSpec.describe RemoveFavouriteRecipeForm, type: :form do
  ...
  describe '.perform' do
    ...
    it "removes the recipe from the user's favourites" do
      perform
      expect(recipe).not_to be_favourited_by(user)
    end

    context "when the recipe is not in the user's favourites" do
      before(:each) do
